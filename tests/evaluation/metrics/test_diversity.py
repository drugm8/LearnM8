...
class TestShannonEntropy:
    def test_zero_compounds_returns_none(self):
        assert _shannon_entropy_from_bit_sum(np.zeros(8), 0) is None

    def test_all_zero_bit_sum_returns_none(self):
        assert _shannon_entropy_from_bit_sum(np.zeros(8), 5) is None
    ...
    def test_uniform_distribution_hits_log2_n_bits(self):
        ...
        result = _shannon_entropy_from_bit_sum(bit_sum, 5)
    ...
    def test_upper_bound_property(self):
        ...
        result = _shannon_entropy_from_bit_sum(bit_sum, n_compounds)
        assert result is not None
...
class TestComputeDiversityMetricsPublic:
    ...
    def test_cycle_zero_batch_equals_cumulative(self, run_cache):
        ...
        assert result["shannon_entropy_diversity_batch"] == pytest.approx(
            result["shannon_entropy_diversity_cumulative"]
        )
