...
def test_mock_pruner_basic_functionality(small_real_compounds):
...
def test_pruner_interface_validation(small_real_compounds):
    ...
    with pytest.raises(PruningError, match="Predictions contain NaN values"):
        pruner.prune(small_real_compounds, bad_predictions)


def test_uncertainty_validation(small_real_compounds):
    ...
    with pytest.raises(PruningError, match="requires uncertainty estimates"):
        uncertainty_pruner.prune(small_real_compounds, predictions)
    
    # Test mismatched uncertainty length
    with pytest.raises(PruningError, match="Uncertainties length"):
        uncertainty_pruner.prune(small_real_compounds, predictions, uncertainties[:5])
    ...
    with pytest.raises(PruningError, match="Uncertainties must be non-negative"):
        uncertainty_pruner.prune(small_real_compounds, predictions, bad_uncertainties)
...
def test_safe_prune_by_indices(small_real_compounds):
    """Test the _safe_prune_by_indices utility method."""
    predictions = np.random.rand(len(small_real_compounds))
    pruner = MockPruner()
    
    # Test boolean indexing
    keep_mask = np.random.choice([True, False], size=len(small_real_compounds))
    pruned = pruner._safe_prune_by_indices(small_real_compounds, keep_mask)
    expected_count = np.sum(keep_mask)
    assert len(pruned) == expected_count
    
    # Test integer indexing
    keep_indices = np.array([0, 2, 4])
    pruned = pruner._safe_prune_by_indices(small_real_compounds, keep_indices)
    assert len(pruned) == 3
    
    # Test empty selection
    empty_indices = np.array([], dtype=int)
    pruned = pruner._safe_prune_by_indices(small_real_compounds, empty_indices)
    assert len(pruned) == 0
