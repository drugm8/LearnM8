...
from examples.oracles import Pharmacophore2DOracle, SimilarityOracle
...
class TestSimilarityOracle:
    def test_initialization_valid(self):
        oracle = SimilarityOracle(reference_smiles='CCO', fingerprint_type='morgan')
    ...
    def test_initialization_invalid_fingerprint(self):
        with pytest.raises(ValueError, match='fingerprint_type must be one of'):
            SimilarityOracle(reference_smiles='CCO', fingerprint_type='invalid')

    def test_initialization_invalid_metric(self):
        with pytest.raises(ValueError, match='metric must be one of'):
            SimilarityOracle(reference_smiles='CCO', metric='invalid')

    def test_measure_basic(self, sample_compounds):
        first_smiles = sample_compounds.get_column('SMILES')[0]
        oracle = SimilarityOracle(
            reference_smiles=first_smiles, fingerprint_type='morgan'
        )
    ...
    def test_measure_invalid_smiles(self):
        compounds = pl.DataFrame(
            {'ID': ['comp1', 'comp2'], 'SMILES': ['CCO', 'INVALID']}
        )
...
class TestPharmacophore2DOracle:
    def test_initialization_valid(self):
        oracle = Pharmacophore2DOracle(reference_smiles='CCO', metric='tanimoto')
        assert oracle.reference_smiles == 'CCO'
    ...
    def test_initialization_invalid_metric(self):
        with pytest.raises(ValueError, match='metric must be one of'):
            Pharmacophore2DOracle(reference_smiles='CCO', metric='invalid')
    ...
    def test_measure_preserves_order(self):
        compounds = pl.DataFrame(
            {'ID': ['z', 'a', 'm'], 'SMILES': ['c1ccccc1O', 'c1ccccc1N', 'c1ccccc1']}
        )
    ...
    def test_different_metrics(self):
        ...
        for metric in ['tanimoto', 'dice']:
            oracle = Pharmacophore2DOracle(reference_smiles='c1ccccc1O', metric=metric)
            result = oracle.measure(compounds, ['similarity'])
...
class TestCDPKitPharmacophoreOracle:
    def test_initialization_valid(self):
        oracle = CDPKitPharmacophoreOracle(
            reference_smiles='CCO', score_type='alignment_score'
        )
        assert oracle.reference_smiles == 'CCO'
    ...
    def test_initialization_invalid_score_type(self):
        with pytest.raises(ValueError, match='score_type must be one of'):
            CDPKitPharmacophoreOracle(reference_smiles='CCO', score_type='invalid')

    def test_measure_basic(self, sample_compounds):
        oracle = CDPKitPharmacophoreOracle(reference_smiles='c1ccccc1O')
    ...
    def test_different_score_types(self):
        ...
        for score_type in ['alignment_score', 'feature_overlap', 'rmsd']:
            oracle = CDPKitPharmacophoreOracle(
                reference_smiles='c1ccccc1O', score_type=score_type
            )
...
class TestVinaOracle:
    ...
    def test_initialization_missing_receptor(self):
        with pytest.raises(FileNotFoundError, match='Receptor file not found'):
            VinaOracle(
                receptor_path='/nonexistent/receptor.pdbqt', center=(0.0, 0.0, 0.0)
            )

    def test_measure_structure(self, mock_receptor_file):
        oracle = VinaOracle(receptor_path=mock_receptor_file, center=(0.0, 0.0, 0.0))

        compounds = pl.DataFrame({'ID': ['comp1'], 'SMILES': ['CCO']})
...
class TestOracleInterface:
    def test_missing_columns(self):
        ...
        bad_df = pl.DataFrame({'wrong_column': ['comp1', 'comp2']})
    ...
    def test_missing_id_column(self):
        ...
        bad_df = pl.DataFrame({'SMILES': ['CCO', 'CCC']})
    ...
    def test_missing_smiles_column(self):
        ...
        bad_df = pl.DataFrame({'ID': ['comp1', 'comp2']})
