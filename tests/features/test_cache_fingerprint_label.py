...
def test_morgan_label_has_uint8packed_suffix():
    ...
    fp_cache: dict = {}
    _, _, label, _ = _get_or_build_fps(['CCO', 'CCC'], feat, None, fp_cache)
...
def test_mordred_label_has_float32_and_fallback_tokens():
    ...
    fp_cache: dict = {}
    _, _, label, _ = _get_or_build_fps(['CCO'], feat, None, fp_cache)
...
def test_no_featurizer_label_is_default_with_dtype_suffix():
    ...
    fp_cache: dict = {}
    _, _, label, _ = _get_or_build_fps(['CCO'], None, None, fp_cache)
...
def test_mqns_label_has_uint8_suffix():
    ...
    _, _, label, _ = _get_or_build_fps(['CCO'], feat, None, fp_cache)
    assert (
        '_uint8' in label and '_uint8packed' not in label and '_float32' not in label
    ), f'got {label!r}'
...
def test_pharmacophore_label_has_csruint16_suffix():
    ...
    feat = create_featurizer('pharmacophore', n_jobs=1)
    fp_cache: dict = {}
    _, _, label, _ = _get_or_build_fps(['CCO'], feat, None, fp_cache)
...
def test_physiochemical_label_has_csruint16_suffix():
    ...
    feat = create_featurizer('physiochemical', n_jobs=1)
    fp_cache: dict = {}
    _, _, label, _ = _get_or_build_fps(['CCO'], feat, None, fp_cache)
    assert '_csruint16' in label, f'got {label!r}'
