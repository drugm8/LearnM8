...
class TestFeaturizerRegistry:
    ...
    def test_registry_contains_all_implemented_featurizers(self):
        ...
        expected_featurizers = {
            'morgan', 'maccs', 'avalon', 'atom_pair', 'topological_torsion',
            'rdkit', 'pubchem', 'mordred', 'whim', 'usr', 'e3fp'
        }

        registry_keys = set(FEATURIZER_REGISTRY.keys())
        assert expected_featurizers.issubset(registry_keys)

    def test_registry_values_are_callable(self):
        """All registry values are callable (classes)."""
        for name, featurizer_class in FEATURIZER_REGISTRY.items():
            assert callable(featurizer_class)
...
class TestExtractFeaturesAPI:
    ...
    def test_extract_features_with_string_featurizer(self, small_real_compounds, tmp_path):
    ...
    def test_extract_features_with_featurizer_instance(self, small_real_compounds, tmp_path):
...
class TestFeaturizerStringVsInstance:
    ...
    def test_string_and_instance_produce_same_features(self, small_real_compounds, tmp_path):
        ...
        featurizer_instance = MorganFeaturizer()
        features_instance = extract_features(smiles, featurizer_instance, tmp_path, n_jobs=1)
    ...
    def test_custom_instance_differs_from_default(self, small_real_compounds, tmp_path):
        ...
        featurizer_custom = MorganFeaturizer(radius=3)
        features_custom = extract_features(smiles, featurizer_custom, tmp_path, n_jobs=1)
...
class TestFeaturizerErrorHandling:
    ...
    def test_extract_features_with_invalid_featurizer_string(self, small_real_compounds, tmp_path):
    ...
    def test_extract_features_with_none_featurizer(self, small_real_compounds, tmp_path):
    ...
    def test_run_active_learning_with_invalid_featurizer(self, small_real_compounds, tmp_path):
