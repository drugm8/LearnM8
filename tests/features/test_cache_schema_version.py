...
def _make_current_v3_file(tmp_path: Path) -> Path:
...
@pytest.mark.unit
def test_fresh_cache_writes_current_schema(tmp_path: Path):
...
def test_v1_file_renamed_with_warning(tmp_path: Path, caplog: pytest.LogCaptureFixture):
    ...
    assert (tmp_path / 'features_morgan.h5.v0.bak').exists()
    assert cache_file.exists()
    with h5py.File(cache_file, 'r') as f:
        assert int(f.attrs['schema_version']) == 3

    rename_warnings = [r for r in caplog.records if 'Renamed non-v3 cache' in r.message]
    assert len(rename_warnings) == 1


@pytest.mark.unit
def test_future_schema_renamed_with_warning(
    ...
    assert (tmp_path / 'features_morgan.h5.v99.bak').exists()
    with h5py.File(cache_file, 'r') as f:
        assert int(f.attrs['schema_version']) == 3
...
def test_bit_count_mismatch_renames_to_dim_bak(
    ...
    cache_file = _make_current_v3_file(tmp_path)
    with h5py.File(cache_file, 'r+') as f:
