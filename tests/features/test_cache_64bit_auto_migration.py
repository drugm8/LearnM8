...
def MorganFeaturizer(**kwargs):
    """Factory-routed Morgan featurizer (collapse-compat shim)."""
    return create_featurizer('morgan', n_jobs=kwargs.pop('n_jobs', 1), **kwargs)
...
def test_migration_emits_info_log(
    ...
    matching = [
        ...
        and 'Migrated v2 (64-bit hash) cache' in rec.getMessage()
    ]
    assert matching, (
        f"Expected a WARNING log mentioning 'Migrated v2 (64-bit hash) cache'; "
        f'got: {[(r.levelname, r.getMessage()) for r in caplog.records]}'
    )
    assert any(str(backup) in rec.getMessage() for rec in matching), (
        f'Expected the backup path {backup} to appear in the WARNING log; '
...
def test_concurrent_v2_migration_race(tmp_path: Path) -> None:
    ...
    assert successes, (
        f'At least one thread must successfully open the v3 cache; results={results}'
    )
    for r in successes:
