...
@pytest.mark.unit
def test_output_dtype_is_s16(morgan: MorganFeaturizer) -> None:
...
@pytest.mark.unit
def test_output_shape_is_one_dimensional(morgan: MorganFeaturizer) -> None:
    arr = _cache_keys_bytes16(SAMPLE_SMILES, morgan)
...
@pytest.mark.unit
def test_determinism_byte_identical_across_calls(morgan: MorganFeaturizer) -> None:
...
@pytest.mark.unit
def test_distinct_smiles_produce_distinct_hashes(morgan: MorganFeaturizer) -> None:
...
def test_same_smiles_different_featurizer_config_differs() -> None:
    binary_feat = MorganFeaturizer(n_jobs=1)
    count_feat = MorganFeaturizer(count=True, n_jobs=1)
...
@pytest.mark.unit
def test_empty_input_returns_empty_s16_array(morgan: MorganFeaturizer) -> None:
...
@pytest.mark.unit
def test_suffix_hoisting_matches_naive_concat(morgan: MorganFeaturizer) -> None:
