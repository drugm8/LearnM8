...
def main():
    strategy_name = 'simulated_annealing'
    ...
    for param_value, result in results.items():
        # Format param_value for filename (handle tuple format)
        if isinstance(param_value, (list, tuple)):
            param_str = f"{param_value[0]}_{param_value[1]}"
        else:
            param_str = str(param_value)
        plot_path = plots_dir / f'validation_plot_{strategy_name}_{param_str}.png'
