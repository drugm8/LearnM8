#!/usr/bin/env python3
"""Expected Improvement (EI) acquisition function validation.

IMPORTANT: Validation results generated before 2025-01-28 are INVALID due to a
critical bug fix in the EI implementation. The bug incorrectly applied
np.sqrt() to uncertainty values that were already standard deviations,
causing EI and PI to produce identical results.

Bug Details:
- Lines 172 in uncertainty_based.py: Used np.sqrt(uncertainties) incorrectly
- Uncertainties from learners are already standard deviations (σ), not variances (σ²)
- This caused inflated Z-scores and made EI behave identically to PI
...
def main():
    ...
    output_dir = Path(runner.global_config['output_base_dir']) / strategy_name
