#!/usr/bin/env python3
"""
Standalone validation script for AmpC 1M dataset using Fastprop with mixed strategy.

This script runs a complete active learning validation on the AmpC 1M dataset:
- Learner: Fastprop (default settings)
...
Usage:
    python validation/scripts/validate_ampc_1M_fastprop_mixed.py

Output:
    validation/reports/large_scale/ampc_1M_fastprop_mixed_validation/
        ├── data/ampc_1M_fastprop_mixed_<timestamp>/
        │   ├── compounds_final.csv
        │   ├── cycle_metrics.csv
        │   └── selection_history.csv
        ├── plots/ampc_1M_fastprop_mixed_validation.png
        └── summary.txt
...
def print_header():
    print("=" * 80)
    print("  AmpC 1M Fastprop Mixed Strategy Validation")
    print("=" * 80)
    print()
...
def print_configuration(compounds_count):
    ...
    print(f"  Learner: Fastprop (default settings)")
...
def save_summary(results, output_dir, elapsed_time, compounds_count):
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write(f"  Plot: {output_dir}/plots/ampc_1M_fastprop_mixed_validation.png\n")
...
def main():
    ...
    LEARNER_NAME = 'Fastprop'

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_output_dir = Path('validation/reports/large_scale/ampc_1M_fastprop_mixed_validation')
    ...
    existing_runs = sorted(data_dir_parent.glob('ampc_1M_fastprop_mixed_*'))
    skip_run = False
    results = None
    ...
    if not skip_run:
        data_output_dir = data_dir_parent / f'ampc_1M_fastprop_mixed_{timestamp}'
    ...
    try:
        ...
        plot_path = plots_dir / 'ampc_1M_fastprop_mixed_validation.png'
