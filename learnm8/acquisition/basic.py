...
class RandomAcquisition(AcquisitionFunction):
    ...
    def select(self, compounds: pd.DataFrame, n_select: int) -> pd.DataFrame:
        ...
        selected['acquisition_score'] = self._rng.uniform(0, 1, size=len(selected))
...
class TopKAcquisition(AcquisitionFunction):
    ...
    def select(self, compounds: pd.DataFrame, n_select: int) -> pd.DataFrame:
        ...
        if self.score_direction == 'higher':
            ...
            sorted_compounds = compounds.sort_values('prediction', ascending=False)
        else:
            # Sort ascending (lowest first)
            sorted_compounds = compounds.sort_values('prediction', ascending=True)
        ...
        if len(top_k_compounds) == actual_n_select:
            selected = top_k_compounds.copy()
        else:
            selected_indices = np.random.choice(
                len(top_k_compounds), size=actual_n_select, replace=False
            )
            selected = top_k_compounds.iloc[selected_indices].copy()
        
        # Add acquisition scores (use prediction values)
        selected['acquisition_score'] = selected['prediction'].values
