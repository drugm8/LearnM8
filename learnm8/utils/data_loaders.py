...
def validate_csv_columns(df: pd.DataFrame, required_columns: list, file_description: str) -> None:
...
def load_benchmark_data(data_path: str, target_column: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if has_activity:
        ...
        unique_values = df['Activity'].dropna().unique()
...
def load_run_data(compounds_path: str, oracle_path: str) -> pd.DataFrame:
    """
    ...
    # Load compound pool
    compound_pool = pd.read_csv(compounds_path)
