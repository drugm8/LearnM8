...
class ChempropEnsemble(EnsembleLearner):
    ...
    def train(self, features, targets, smiles=None):
        ...
        Args:
                features: Optional pre-computed molecular descriptors (x_d),
                        forwarded to every member model for hybrid mode. May be None.
        ...
        if len(smiles) != len(targets):
            raise ValueError(
                f'SMILES and targets must have same length: {len(smiles)} vs {len(targets)}'
            )
