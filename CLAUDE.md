...
- **027 Adaptive annealing schedule (v0.12.0 — BREAKING behaviour)** — `SimulatedAnnealingAcquisition` degenerated to greedy in production and reported nothing. Energy is `-prediction`, so temperature carries the **target's units**, but `initial_temp` defaulted to a bare `1.0`; on real AmpC cycle-9 predictions (880,541 rows, `E[ΔE⁺]=3.63`) mean acceptance at `T₀=1.0` was **0.178** and the chain froze. `max_iterations=1000` was independent of `n_select`, so the shortfall went to silent `argsort(prediction)` backfill: measured **99.8% backfill / 99.9% greedy overlap** at `n_select=10000`. **Fix**: `initial_temp`/`final_temp`/`max_iterations` now default to `None` = derive per cycle (REQ-1). Temperatures solved by **Ben-Ameur fixed point** (`_calibrate_temperature`) so the *realized* mean uphill acceptance hits `SA_TARGET_UPHILL_ACCEPTANCE=0.8` / `SA_FINAL_UPHILL_ACCEPTANCE=0.01`; the closed form `E[ΔE⁺]/−ln χ` **seeds only** (Jensen: `exp` convex ⇒ overshoot grows with CV of ΔE — 0.009 at CV 0.71, 0.039 at CV 2.22, vs `SA_CALIBRATION_TOL=0.01`). Gaps sampled via `_sample_deltas` → `_propose`, i.e. literally the walk's own proposal path, never the global σ (a ±50-rank `'score_band'` window and a uniform draw differ by orders of magnitude on identical predictions). **Step budget** `_derive_step_budget(n_select, pool_size, mean_acceptance)` inverts expected-unique-visits `A = ln(1−U/N)/ln(1−1/N)` (coupon-collector; ignoring it under-budgets at high exhaustion) then divides by a **pilot-MEASURED** χ̄ (`_pilot_acceptance_rate`, `SA_PILOT_STEPS=300 × SA_PILOT_CHAINS=16`). **Deviation from spec REQ-4, approved:** the spec's uniform-origin χ̄ estimate reads 0.657 while the walk actually runs at 0.208 under `'random'` — a chain *descends* into the low-energy tail where every uniform proposal is a large uphill jump — which under-budgeted 3× and produced **62.8% backfill on the default strategy**, tripping REQ-13's own `AcquisitionError`. Pilot-measured χ̄ gives **0.0%** at `n_select ∈ {1000, 10000}`; `'score_band'` unchanged (pilot 0.662 ≈ uniform 0.657). **Scaling**: `R` independent chains × `L = min(SA_CHAIN_LENGTH=2000, total)` steps, vectorized over chains (`_run_chains`, `_acceptance_mask`, `_propose`); Python loop is `L` iterations regardless of `n_select`/pool size; chains start at `R` independent positions (REQ-8) so they explore different basins. Measured **13× faster** at 880k×10k: 0.07 s vs 0.92 s. **`max_iterations` semantics changed**: an explicit value is now the TOTAL budget across chains (`R·L ≤ max_iterations`), floor-divided; a derived budget rounds up. Clamped at `SA_MAX_TOTAL_STEPS=2e7` with a WARNING. **Backfill visibility**: `last_backfill_fraction` attribute + `metrics['acquisition_backfill_fraction']` (threaded through `core/cycle.py:_select_and_measure`, now a 6-tuple; `None` for non-SA strategies — no fabricated zeros); >`SA_BACKFILL_WARN=0.10` logs WARNING, >`SA_BACKFILL_ERROR=0.50` raises `AcquisitionError` (WHAT/WHY/HOW). Full-pool path reports 0.0, not 1.0 (REQ-14). **Related fixes**: `select()` returns rows **best-first** by `acquisition_score` (was pool order, so `cycle.py`'s `.head(batch_size)` dropped arbitrary not worst); a position is recorded visited **only on acceptance** (was unconditional, re-logging on every rejection); dead `best_energy` removed; `_metropolis_accept` is now a thin scalar wrapper over `_acceptance_mask` (one implementation, REQ-18); `_calculate_energy` wraps vectorized `_energies`. **`supports_streaming()` stays `False`** — SA is a path-dependent walk over the full pool, not expressible as the pointwise `score_chunk` contract, so it keeps the legacy cycle path (REQ-10, restated so it is not read as an oversight). **RNG-stream break**: every seeded SA selection differs from v0.11 (different algorithm; calibration sample + pilot walk drawn before the chains). Determinism within 0.12.0 preserved and tested. **Spec deviation on T022**: the specified acceptance test (measured uphill acceptance over the first `L/10` walk steps within ±0.05 of 0.8) measures a different quantity than REQ-3 calibrates — T decays and the chain relocates across that window, so no `T₀` satisfies it (measured 0.56–0.91); restated to measure realized acceptance **at the calibrated `T₀`**, which is what REQ-3 solves for and still catches the frozen chain (0.162 at old `T₀=1.0` vs 0.823 derived). **Archived A-08 runs are NOT recoverable** — they are greedy runs mislabelled as SA and must be re-run if SA is to be reported (re-running out of scope).
